...
import org.firstinspires.ftc.teamcode.roadrunner.drive.TrackingWheelLocalizerImpl;

/**
 * Opmode designed to assist the user in tuning the `TrackingWheelLocalizerImpl`'s
 * LATERAL_DISTANCE value. The LATERAL_DISTANCE is the center-to-center distance of the parallel
 * wheels.
 *
 * Tuning Routine:
 *
 * 1. Set the LATERAL_DISTANCE value in TrackingWheelLocalizerImpl.java to the physical
 ...
 * TrackingWheelLocalizerImpl.java class.
 *
...
public class TrackingWheelLateralDistanceTuner extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (!(drive.getLocalizer() instanceof TrackingWheelLocalizerImpl)) {
            RobotLog.setGlobalErrorMsg("TrackingWheelLocalizerImpl is not being set in the "
                    + "drive class. Ensure that \"setLocalizer(new TrackingWheelLocalizerImpl"
                    + "(hardwareMap));\" is called in MecanumDriveImpl.java");
        }
        ...
        while (!isStopRequested() && !tuningFinished) {
            ...
            telemetry.addData("Unghi: ",imu.getAngularOrientation().firstAngle);
        ...
        telemetry.addLine("Effective LATERAL_DISTANCE: " +
                (headingAccumulator / (NUM_TURNS * Math.PI * 2)) * TrackingWheelLocalizerImpl.LATERAL_DISTANCE);
